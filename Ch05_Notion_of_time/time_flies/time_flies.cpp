...
void time_flies::time_thread(void) {
  ...
  for (t_DELAY=2*t_1HR;t_DELAY>=t_1FS;t_DELAY/=10) {
    ...
    cout << "INFO: Time now " << display_hms(sc_time_stamp().to_seconds()) << "!" << endl;
  }//endfor

  wait(SC_ZERO_TIME); //SC_ZERO_TIME is for update phase
  cout << "INFO: wait(SC_ZERO_TIME)" << endl;
  //cout << "INFO: Time still " << display_hms(sc_simulation_time()) << "!" << endl;
  cout << "INFO: Time still " << display_hms(sc_time_stamp().to_seconds()) << "!" << endl;
  #ifdef USE_STOP
...
// note: input arg is seconds in double
